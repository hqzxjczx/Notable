...
## 七、配套资源（qoderclicn：AI 编码防封）

> `notes/qoderclicn/` 下是一套**互补**材料：聚焦 **Claude Code / Codex CLI 的账号安全与防封**，
> 底层「环境一致性」原理与本方案完全一致（注册信息 → IP 出口 → 时区 → DNS → 语言 → 支付区域）。
> 可作为本隐私方案的「AI 编码用例」配套参考。

| 资源 | 内容 | 与本方案关系 |
|---|---|---|
| `qoderclicn/ai-coding-security-privacy-guide.md` | AI 编码工具安全隐私与防封指南（macOS/Windows WSL2/Apple Container 配置片段） | 原理同源，可互为补充阅读 |
| `qoderclicn/switch-env-macos.sh` | macOS 多区域(us/jp/uk/sg/cn)一键切换 | **架构冲突**：假设 TUN + 手设 DNS 1.1.1.1/1.0.0.1 + 端口 7897 |
| `qoderclicn/switch-env-wsl2.sh` | WSL2 多区域切换（并同步 Windows 宿主机时区/DNS） | 与 Windows/TUN 架构一致，可直接参考 |
| `qoderclicn/switch-env-container.sh` | 建 Docker/OrbStack/Colima 隔离容器跑 Claude/Codex | 容器隔离思路可参考；运行时与 Apple Container 不同 |

### 直接套用前必须注意的差异
1. **端口**：qoderclicn 脚本原用 7890，现已对齐为本方案 SakuraCat 的 **7897**（`PROXY_PORT` 已改）。端口差异已消除；其余差异见下条 DNS/TUN。
2. **macOS DNS**：qoderclicn 在 macOS **手设** `1.1.1.1/1.0.0.1` 并假设开 **TUN** 防 WebRTC；本方案 macOS 是 **代理 7897 + WARP DNS（不手设 DNS、不开 TUN）**。故 `switch-env-macos.sh` 的 DNS/TUN 部分**不适用于本方案**，仅其「带标记的 .zshrc 块管理 / 多区域映射」技术可借鉴（已融入 `macos-privacy.sh`）。
3. **Windows / WSL2**：qoderclicn 的 TUN 假设与本文档一致，脚本可直接参考（端口已对齐 7897）。

> 相关：本方案已借鉴 `switch-env-macos.sh` 的「带标记 .zshrc 块管理 / 全网卡 IPv6 / 状态打印」技术，升级到 `macos-privacy.sh`（端口 7897 + WARP DNS）。
